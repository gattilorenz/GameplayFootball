...
Referee::Referee(Match *match) : match(match) {
    ...
    soundBufferRes = ResourceManagerPool::GetInstance().GetManager<SoundBuffer>(e_ResourceType_SoundBuffer)->Fetch(
            "media/sounds/whistle3.wav", true, true);
...
void Referee::Process() {
    ...
    if (match->IsInPlay() && !match->IsInSetPiece()) {
        ...
        if (!match->IsInPlay() && !match->IsInSetPiece() && buffer.active) {

            if (buffer.stopTime + 300 == match->GetActualTime_ms() && !buffer.endPhase &&
                buffer.desiredSetPiece != e_SetPiece_KickOff) {
                whistle[1]->SetGain(0.3 * GetConfiguration()->GetReal("audio_volume", 0.5));
    ...
    if (match->IsInSetPiece()) {
        ...
        if (buffer.taker->TouchAnim() && !buffer.taker->TouchPending()) {
            ...
            if (!skip) {
                match->StoreMatchAction(action, buffer.taker->GetPlayerData(), buffer.taker->GetTeam()->GetTeamData(),
...
void Referee::BallTouched() {
    ...
    if (match->IsInPlay() && !match->IsInSetPiece() && !buffer.active &&
        ...
        auto playerIter = offsidePlayers.begin();
        while (playerIter != offsidePlayers.end()) {
            if (match->GetTeam(lastTouchTeamID)->GetLastTouchPlayer() == playerIter->first) {
                ...
                if (!CheckFoul()) {
                    ...
                    match->StoreMatchAction("Buitenspel", playerIter->first->GetPlayerData(),
        ...
        if (match->GetActualTime_ms() - tripper->GetLastTouchTime_ms() > 600 &&
            ...
            if (tripper->TouchAnim()) {
                severity = std::pow(clamp(fabs(tripper->GetTouchFrame() -
                           ...
                           0.5;
                severity +=
                        NormalizedClamp((match->GetBall()->Predict(0) - tripper->GetTouchPos()).GetLength(), 0.0, 2.0) *
                        0.5;
            }
            // from behind?
            severity += (tripee->GetPosition() - tripper->GetPosition()).GetNormalized(0).GetDotProduct(
                    tripee->GetDirectionVec()) * 0.5 + 0.5;
...
bool Referee::CheckFoul() {
    ...
    if (foul.foulType != 0) {
        if (fabs(foul.foulPosition.coords[1]) < 20.15 - lineHalfW &&
            foul.foulPosition.coords[0] * -foul.foulVictim->GetTeam()->GetSide() > pitchHalfW - 16.5 + lineHalfW)
            penalty = true;
    }
    ...
    if (foul.foulType != 0 && !foul.advantage && !foul.hasBeenProcessed) {
        ...
        if (!penalty) {
            ...
            match->StoreMatchAction("Overtreding (vrije trap)", foul.foulPlayer->GetPlayerData(),
            ...
            match->StoreMatchAction("Overtreding (strafschop)", foul.foulPlayer->GetPlayerData(),
        ...
        if (foul.foulType == 2) {
            ...
            match->StoreMatchAction("Geel", foul.foulPlayer->GetPlayerData(), foul.foulPlayer->GetTeam()->GetTeamData(),
                                    match->GetMatchTime_ms());
        }
        if (foul.foulType == 3) {
            spamMessage.append(" red card!!!");
            foul.foulPlayer->GiveRedCard(match->GetActualTime_ms() + 6000); // need to find out proper moment
            match->StoreMatchAction("Rood", foul.foulPlayer->GetPlayerData(), foul.foulPlayer->GetTeam()->GetTeamData(),
                                    match->GetMatchTime_ms());
        }
