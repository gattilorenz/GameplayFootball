...
void Player::Activate(boost::intrusive_ptr <Node> humanoidSourceNode, boost::intrusive_ptr <Node> fullbodySourceNode,
    ...
    nameCaption = new Gui2Caption(GetMenuTask()->GetWindowManager(), "game_player_name_" + int_to_str(id), 0, 0, 1, 2.0,
                                  playerData->GetLastName());
    nameCaption->SetTransparency(0.3f);
    GetMenuTask()->GetWindowManager()->GetRoot()->AddView(nameCaption);
    debugCaption = new Gui2Caption(GetMenuTask()->GetWindowManager(), "game_player_debug_" + int_to_str(id), 0, 0, 1,
                                   1.6, "debug");
    GetMenuTask()->GetWindowManager()->GetRoot()->AddView(debugCaption);

    CastHumanoid()->ResetPosition(GetFormationEntry().position * 25 * Vector3(-team->GetSide(), -team->GetSide(), 0),
                                  Vector3(0));
...
void Player::UpdatePossessionStats(bool onInterval) {
    ...
    // default
    timeNeededToGetToBall_ms = std::max(ballPredictionSize_ms, (unsigned int) (round(
            (match->GetBall()->Predict(ballPredictionSize_ms - 10).Get2D() -
             (GetPosition() + GetMovement() * 0.2f)).GetLength() / (GetMaxVelocity() * 0.75f) * 1000)));
    ...
    for (unsigned int ms = startTime_ms; ms < ballPredictionSize_ms; ms += timeStep_ms) {
        if (match->GetBall()->Predict(ms).coords[2] < 1.5f) {
            TimeNeeded result = AI_GetTimeNeededForDistance_ms(GetPosition(), GetMovement(),
                                                               match->GetBall()->Predict(ms).Get2D(), GetMaxVelocity(),
                                                               precise, ms, GetDebug());
        ...
        if (!refine) {
            float balldist =
                    (GetPosition() - match->GetBall()->Predict(ms).Get2D()).GetLength() + 0.2f; // add a little buffer
    ...
    if (timeNeededToGetToBall_ms < defaultTouchOffset_ms) {
        ...
        timeNeededToGetToBall_ms = NormalizedClamp(((GetPosition() + GetMovement() * (defaultTouchOffset_ms * 0.001)) -
                                                    match->GetBall()->Predict(
                                                            defaultTouchOffset_ms).Get2D()).GetLength(), 0.0f, 0.6f) *
    ...
    this->hasBestPossession = hasPossession && match->GetTeam(abs(team->GetID() - 1))->GetTimeNeededToGetToBall_ms() >
                                               this->GetTimeNeededToGetToBall_ms();
    this->hasUniquePossession = hasPossession && !match->GetTeam(abs(team->GetID() -
                                                                     1))->HasPossession(); // todo: this should be in a seperate function, to be called after the other team's timetoball function already ran.
...
void Player::Process() {
    ...
    if (isActive) {
        ...
        if (cards > 1 && cardEffectiveTime_ms <= match->GetActualTime_ms()) {
            ...
            // store a red card
            int actionCodes[4] = {4096, 0, 0, 3};
            match->StoreMatchAction("Rood (2x geel)", actionCodes, playerData, team->GetTeamData(), match->GetMatchTime_ms());
        }
...
//      Vector3 p2 = GetProjectedCoord(match->GetPlayer(GetManMarkingID())->GetGeomPosition().Get2D(), match->GetCamera());
            Vector3 color;
            if (team->GetID() == 0) color = Vector3(255, 255, 255);
            else
...
//      GetMenuTask()->GetWindowManager()->GetCoordinates(p2.coords[0], p2.coords[1], 1, 1, x2, y2, dud1, dud2);
    ...
    if (fetchedbuf_nameCaptionShowCondition) {
        ...
        Vector3 captionPos3D = GetProjectedCoord(GetGeomPosition() + Vector3(0, 0.5f, 2.4f),
                                                 match->GetCamera()); // geom pos because in Put2D, we cannot access normal class vars (because multithreading)
        float w, h;
...
void Player::_CalculateTacticalSituation() {
    ...
    tacticalSituation.forwardSpaceRating = AI_CalculateFreeSpace(match, mentalImage, team->GetID(), checkPos, 5.0f,
                                                                 time_sec, true); // FREESPACE :D :D
    ...
    tacticalSituation.forwardRating = 1.0f -
                                      clamp((Vector3(pitchHalfW * -team->GetSide(), 0, 0) - GetPosition()).GetLength() /
